...
_CURRENT_PERIODIC_MODE = CURRENT_L32_DERIVED_MODE
...
_PROMOTABLE_PERIODIC_RODS = ("L2", "L3", "L5", "L6", "L7", "L8")
...
def _staged_feasibility_loss(
    ...
    try:
        ...
        for frame in range(n_frames):
            ...
            m = k + np.array(
                [-np.sin(theta_m[frame]) * p.L12, np.cos(theta_m[frame]) * p.L12],
                dtype=float,
            )
            mk = k - m
            mj = j - m
...
def _run_pso_refinement(
    ...
    for iteration in range(iteration_count):
        if (
            time.monotonic() >= deadline
            or evaluation_counter[0] >= int(config.max_evaluations)
            or _goal_reached(global_candidate.metrics, config.target_rmse_mm)
        ):
            break
        ...
        if _goal_reached(global_candidate.metrics, config.target_rmse_mm):
            break
...
def _save_optimization_outputs(
    ...
    np.savez_compressed(
        ...
        variable_schema=(
            ...
            + f"TargetPose_{config.target_pose_mode}_no_anchor"
        ),
    )
    ...
    summary = {
        ...
        "no_external_anchor": True,
        "same_campaign_continuation": checkpoint_path is not None,
        "continuation_checkpoint": (
            str(checkpoint_path) if checkpoint_path is not None else None
        ),
        "continuation_space_mapping": (
        ...
        "initial_source": (
            (
                "same formal campaign checkpoint; no historical optimizer result, "
                "population, covariance or partition tree loaded"
            )
            if checkpoint_path is not None
            else
...
def _validate_optimization_input_contract(config: _OptimizeConfig) -> dict[str, Any]:
    ...
    if (
        int(design.get("stored_coordinate_count", -1)) != 55
        or int(design.get("independent_search_dimension", -1)) != 54
        or design.get("derived_stored_coordinates") != ["B_Z_C3c"]
    ):
        raise ValueError("optimization input contract has the wrong 55/54 design-space contract")
    target_ty = next(
    ...
    if config.start_checkpoint_path or expected_runtime.get("start_checkpoint_path") is not None:
        raise ValueError("this input contract requires a cold start without a checkpoint anchor")
...
def _run_full_optimization_impl(
    ...
    if checkpoint_path is None:
        ...
        # 累计续跑只读取同一任务上一小时保存的最优向量，不读取种群、
        # 协方差、分区树或其他算法结果；本小时重新建立区域树。
        if not checkpoint_path.is_file():
            raise FileNotFoundError(
                f"Continuation checkpoint does not exist: {checkpoint_path}"
            )
        ...
        if recorder is not None:
            recorder.record_candidate(
                ...
                round_index=-1,
                metadata={
                    "purpose": "same-campaign checkpoint continuation",
                    "checkpoint_path": str(checkpoint_path),
                    "space_mapping": continuation_mapping,
                },
            )
            recorder.record_region_event(
                "same_campaign_continuation_loaded",
                {
                    "checkpoint_path": str(checkpoint_path),
                    "space_mapping": continuation_mapping,
        ...
        if not initial.valid:
            raise RuntimeError(
                "Continuation checkpoint is invalid under the current model: "
                f"{checkpoint_path}"
            )
    ...
    for round_index in range(int(config.max_rounds)):
        if (
            time.monotonic() >= deadline
            or evaluation_counter[0] >= int(config.max_evaluations)
            or _goal_reached(best.metrics, config.target_rmse_mm)
        ):
            break
        ...
        for selected_rank, region in enumerate(selected):
            ...
            if valid:
                ...
                if local_best.score < best.score:
                    best = local_best
                if _goal_reached(best.metrics, config.target_rmse_mm):
                    break
        ...
        if (
            ...
            and not _goal_reached(best.metrics, config.target_rmse_mm)
        ):
