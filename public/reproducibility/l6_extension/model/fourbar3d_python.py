...
def _fourbar_direct_b_l6(
    ...
    if time_varying_lengths:
        for name, values in time_varying_lengths.items():
            ...
            if str(name) not in {"L2", "L3", "L5", "L6", "L7", "L8", "L12"} and float(np.ptp(curve)) > 1e-10:
                raise FourBarError(
                    f"{name} must be constant in the selected periodic-rod mode."
                )
    ...
    l12_curve = length_curves.get("L12")
    if l12_curve is None:
        l12_curve = np.full(n, p.L12, dtype=float)
    ...
    for i in range(n):
        ...
        theta17[i], theta20[i], theta19[i] = qa2(
            p.L10, p.L11, p.L13, L12i, theta21[i],
    ...
    for i in range(n):
        ...
        Position[M, :2] = [
            Position[K, 0] - np.sin(theta_m[i]) * L12i,
            Position[K, 1] + np.cos(theta_m[i]) * L12i,
        ]
...
def _find_current_dir(model_dir: Path) -> Path:
    for candidate in (model_dir, *model_dir.parents):
        ...
        if (
            ...
            and (input_dir / "Length_normalized_target_initialized_equal_arc_76_mm.csv").is_file()
            and (input_dir / "Length_normalized_target_initialized_equal_arc_76_metadata.json").is_file()
        ):
            return candidate
...
L3_L8_L12_PERIODIC3_L5_FIXED_STATIC_NAMES = (
    "L1", "L3", "L8", "L12", "L32", "H_finger", "Lf1", "L_CZ",
)
L3_L8_L12_PERIODIC3_L5_FIXED_ACTIVE_STATIC_NAMES = tuple(
    name for name in STATIC_NAMES
    if name not in L3_L8_L12_PERIODIC3_L5_FIXED_STATIC_NAMES
)
...
L12_FOURIER_NAMES = ("L12_C0", "L12_C1c", "L12_C1s", "L12_C2c", "L12_C2s")
...
L12_FOURIER3_NAMES = L12_FOURIER_NAMES + ("L12_C3c", "L12_C3s")
...
L12_PERIODIC_L32_DERIVED_MODE = "l3_l8_l12_periodic3_l5_fixed_l32_derived"
PERIODIC_ROD_SCREEN_MODES = {
    "l3_l8_l2_periodic3_l32_derived": "L2",
    "l3_l8_l5_periodic3_l32_derived": "L5",
    "l3_l8_l6_periodic3_l32_derived": "L6",
    "l3_l8_l7_periodic3_l32_derived": "L7",
    L12_PERIODIC_L32_DERIVED_MODE: "L12",
}
...
class DesignState:
    ...
    l12_values: np.ndarray | None = None
    l12_fourier_coeff: np.ndarray | None = None
    zc_values: np.ndarray | None = None
...
def load_initialized_equal_arc_target(
    ...
    if not workbook.is_file():
        # Portable analysis capsules preserve the source filename and hash but
        # may not preserve the original absolute workstation path.
        portable_workbook = meta_path.parent / workbook.name
        if portable_workbook.is_file():
            workbook = portable_workbook
    if not workbook.is_file():
...
def build_design_space(
    ...
    if periodic_length_mode == "l6_periodic":
    ...
    elif periodic_length_mode == L12_PERIODIC_L32_DERIVED_MODE:
        active_static_names = L3_L8_L12_PERIODIC3_L5_FIXED_ACTIVE_STATIC_NAMES
        periodic_blocks = (
            ("L3", L3_FOURIER3_NAMES),
            ("L8", L8_FOURIER3_NAMES),
            ("L12", L12_FOURIER3_NAMES),
        )
    elif periodic_length_mode in PERIODIC_ROD_SCREEN_MODES:
        added_rod = PERIODIC_ROD_SCREEN_MODES[periodic_length_mode]
        fixed_static_names = set(L3_L8_PERIODIC3_L5_FIXED_STATIC_NAMES)
        fixed_static_names.add(added_rod)
        active_static_names = tuple(
            name for name in STATIC_NAMES if name not in fixed_static_names
        )
        block_names_by_rod = {
            "L2": L2_FOURIER3_NAMES,
            "L5": L5_FOURIER3_NAMES,
        ...
        periodic_blocks = (
            ("L3", L3_FOURIER3_NAMES),
            ("L8", L8_FOURIER3_NAMES),
            (added_rod, block_names_by_rod[added_rod]),
        )
...
def repair_design_vector(
    ...
    periodic_starts = [
        ...
        for name in (
            ...
            "L5_C0", "L7_C0", "L8_C0", "L12_C0", "ZC_C0",
        )
        if name in index
    ]
...
def decode_design_vector(
    ...
    coefficient_defaults = {
        ...
        "L12": constant_periodic(params.L12),
    ...
    l12_coeff = periodic_coeff_by_rod.get("L12", coefficient_defaults["L12"]).copy()
    ...
    l12_values = periodic_basis(periodic_phase, l12_coeff.size) @ l12_coeff
    ...
    return DesignState(
        ...
        l12_fourier_coeff=l12_coeff.copy(),
        zc_values=zc_values,
...
def load_checkpoint_state(path: Path | str) -> DesignState:
    ...
    with np.load(Path(path), allow_pickle=True) as saved:
        ...
        l12_default = float(static[STATIC_NAMES.index("L12")])
        ...
        return DesignState(
            ...
            l12_values=np.asarray(
                saved.get("l12_values", np.full(phase_count, l12_default)), dtype=float
            ).reshape(-1),
            l12_fourier_coeff=np.asarray(
                saved.get(
                    "l12_fourier_coeff",
                    fit_fourier5(
                        np.arange(phase_count) / phase_count,
                        saved.get("l12_values", np.full(phase_count, l12_default)),
                    ),
                ),
                dtype=float,
            ).reshape(-1),
...
def evaluate_design_state(
    ...
    try:
        ...
        l12_values = (
            np.asarray(state.l12_values, dtype=float).reshape(-1)
            if state.l12_values is not None else np.zeros(0, dtype=float)
        )
        zc_values = (
        ...
        if l32_derived_periodic_mode:
            ...
            fixed_curves = [
                (name, curve)
                for name, curve in curve_by_name.items()
                if f"{name}_C0" not in state.variable_names
            ]
        ...
        if fixed_moving_lengths:
            ...
            if l12_values.size:
                l12_values = np.full(76, float(np.mean(l12_values)))
            if zc_values.size:
            ...
            if l12_values.size:
                static[STATIC_NAMES.index("L12")] = float(l12_values[0])
            if zc_values.size:
        ...
        if l12_values.size == 0:
            l12_values = np.full(76, params.L12, dtype=float)
